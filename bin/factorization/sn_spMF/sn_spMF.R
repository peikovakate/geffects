...
sn_spMF <- function(xfn, wfn, K, alpha1, lambda1, outputdir, 
			...
			draw_factor = 1,  colors_file="", rows_to_read = NULL){
	...
	if(compute_cophenet == 1){
		...
		for(run_idx in runs){
			...
			print(paste(rep("#",30), collapse = ''))
			cat('\n')
			run_one_iter(X, W, option, outputdir, paste0(Fn, '_Run',run_idx), draw_factor, colors_file);
		}

	}else{
		run_one_iter(X, W, option, outputdir, Fn, draw_factor, colors_file);
	}
...
run_one_iter <- function(X, W, option, outputdir, outputFn, draw_factor, colors_file){
        ...
        if(draw_factor){
                plot_factor_matrix(factorFn, colors_file)
                plot_factor(factorFn, colors_file)
        }
}
