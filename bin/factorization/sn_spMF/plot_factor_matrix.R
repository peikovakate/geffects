...
plot_factor_matrix <- function(Factor_fn, colors_file){
  ### gtex colors for the tissues
  gtex_col = read_tsv(colors_file)
  ...
  if(sum(!rownames(factor_matrix) %in% gtex_col$tissue) > 0){
    ...
    print(rownames(factor_matrix)[!rownames(factor_matrix) %in% gtex_col$tissue])
...
plot_factor <- function(Factor_fn, colors_file, plot_panel_names = F){
  factor_matrix = read.table(Factor_fn, sep='\t', stringsAsFactors = F, header = T, row.names = 1)
  ### gtex colors for the tissues
  gtex_col = read_tsv(colors_file)
  gtex_col = gtex_col[order(gtex_col$tissue), ]
