...
option_list = list(
	...
	make_option(c("-r", "--runidx"), type="integer", default=-1,help="The ith runs to compute cophenetic coefficient"),
	make_option(c("-v", "--verbose"), type="integer", default=1,help="Display the progress"),
	make_option(c("-p", "--plot"), type="integer", default=1,help="Plot the factor matrix or not"),
	make_option(c("-s", "--colors"), type="character", default="colors.txt", help="file with tissue, color columns"))
...
sn_spMF(opt$xfilename, 
		...
		run_idx = opt$runidx,
		draw_factor = opt$plot,
		colors_file = opt$colors,
		rows_to_read = NULL) # rows_to_read: for testing purpose. can be set small to make sure the pipeline runs
