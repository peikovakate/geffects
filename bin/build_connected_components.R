...
parser <- optparse::add_option(parser, c("-m", "--fine_mapping"),
  type = "character",
  default = "susie_ge.tsv",
  help = "tsv with susie files"
)
...
read_fine_mapping <- function(files) {
  print("Loading fine-mapped files")
  tbls <- mapply(function(file, qtl_group_name) {
    tbl <- readr::read_tsv(file, progress = F, col_types = readr::cols()) %>%
      dplyr::mutate(qtl_group = qtl_group_name)
    return(tbl)
  }, files$path, files$qtl_group, SIMPLIFY = F)
...
get_cc <- function(phenotype_group) {
  ...
  components_sets <- 1:igraph::components(g)$no %>%
    lapply(function(x) {
      return(names(variants)[igraph::components(g)$membership == x])
    })
...
susie_files <- readr::read_tsv(args$fine_mapping, col_names = c("path", "qtl_group"))
tbl_fine_mapped <- read_fine_mapping(susie_files)$credible_sets
...
# coords <- variants_connected_components %>%
